...
/**
 * @brief An APIJSONHandler subclass for creating feeds from a JSON HTTP body
 * @details The HTTP body is expected to be a JSON dictionary with a single
 *          array pointed to by the key 'feeds'
 */
class APIAddFeedsHandler extends APIJSONHandler
{
  ...
  private function insert_sample(APIAddFeed $feed, $mid)
  {
    ...
    $afterid = mysqli_insert_id($db);

    $feed->setSIDs($nextid, $afterid);
